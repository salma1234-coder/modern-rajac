...
// Admin: Get all users
app.get("/api/admin/users", apiLimiter, isAdmin, async (req, res) => {
    try {
        await db.read();
...
app.post("/api/admin/users", apiLimiter, isAdmin, async (req, res) => {
    try {
        ...
        res.json({ success: true, user: { id: newUser.id, username: newUser.username, fullName: newUser.fullName } });
    } catch (error) {
...
// Admin: Update user
app.put("/api/admin/users/:id", apiLimiter, isAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
// Admin: Get login records
app.get("/api/admin/login-records", apiLimiter, isAdmin, async (req, res) => {
    try {
        await db.read();
        res.json(db.data.loginRecords);
    } catch (error) {
    ...
    try {
        ...
        for (let i = 1; i <= count; i++) {
            ...
            if (passwordMode === 'auto') {
                ...
                password = passwords[0]; // Same password for all
...
app.get("/api/admin/teachers", apiLimiter, isAdmin, async (req, res) => {
    try {
        await db.read();
...
app.delete("/api/admin/teachers/:id", apiLimiter, isAdmin, async (req, res) => {
    try {
        ...
        db.data.users.splice(teacherIndex, 1);
        await db.write();
...
// Admin: Export login records to CSV
